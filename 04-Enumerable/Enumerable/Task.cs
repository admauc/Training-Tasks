...
namespace EnumerableTask {
    ...
    public class Task {
        ...
        return data.Select(x => x != null ? x.ToUpperInvariant() : null);
        ...
        return data.Where(x => x != null)
            .Where(x => x.IndexOf(prefix, StringComparison.CurrentCultureIgnoreCase) == 0);
        }
        ...
        public IEnumerable<char> GetUsedChars(IEnumerable<string> data) {
               return data.Where(x => !string.IsNullOrEmpty(x)).SelectMany(x => x).Distinct();
        }
        ...
        public string GetStringOfSequence<T>(IEnumerable<T> data) {
               return string.Join(",", data.Select(x => x == null ? "null" : x.ToString()).ToArray());
        ...
        public string GetFirstContainsFirst(IEnumerable<string> data) {
               return data.Where(x => x != null).
               FirstOrDefault(x => x.IndexOf("First", StringComparison.CurrentCultureIgnoreCase) > 0);
        }
        ...
        public int GetCountOfStringsWithLengthEqualsTo3(IEnumerable<string> data) {
               return data.Distinct().Count(x => x != null && x.Length == 3);
        }
        ...
        public IEnumerable<Tuple<string,int>> GetCountOfStrings(IEnumerable<string> data) {
               return data.GroupBy(s => s).Select(x => new Tuple<string, int>(x.Key, x.Count()));
        }
        ...
        public int GetCountOfStringsWithMaxLength(IEnumerable<string> data) {
               var max = data.DefaultIfEmpty("").Max(y => !string.IsNullOrEmpty(y) ? y.Length : 0);
               return data.Count(x => (x != null ? x.Length : 0) == max);
        }
        ...
        public int GetDigitCharsCount(string data) {
               return data.Count(char.IsNumber);
        }
        ...
        public IEnumerable<string> GetIEnumerableTypesNames(Assembly assembly) {
            ...
            return assembly.GetExportedTypes().Where(x => x.GetInterfaces().Any(z => z.Name == "IEnumerable")).Select(x => x.Name).Distinct();
        }
        ...
        // TODO : Implement GetQuarterSales
            var z = new[] { 0, 0, 0, 0 };
            sales.Select(x => z[((x.Item1.Month - 1) / 3)] += x.Item2).ToArray();
            return z;
        }
        ...
        public IEnumerable<string> SortDigitNamesByNumericOrder(IEnumerable<string> data) {
                ...
                return data.OrderBy(x => Array.IndexOf(digits, x));
        }
        ...
        var start = data.DefaultIfEmpty("").First();
        return data.Where(x => !string.IsNullOrEmpty(x)).DefaultIfEmpty("").Select(start.Intersect).Last();
        }
        ...
        public int GetSumOfAllInts(object[] data) {
               return data.OfType<int>().DefaultIfEmpty().Aggregate((sum, next) => sum + next);
            }
        ...
        public int GetTotalStringsLength(IEnumerable<string> data) {
               return data.OfType<string>().Aggregate(0, (sum, next) => sum + next.Length);
            }
        ...
        public bool AreNumericListsEqual(IEnumerable<int> integers, IEnumerable<double> doubles) {
               return integers.Zip(doubles, (i, d) => i - (int)d == 0).All(x => x);
            }
        ...
        public IEnumerable<int> GetSumOfVectors(IEnumerable<int> vector1, IEnumerable<int> vector2) {
               return vector1.Zip(vector2, (i, i1) => i + i1);
            }
        ...
        public int GetProductOfVectors(IEnumerable<int> vector1, IEnumerable<int> vector2) {
               return vector1.Zip(vector2, (i, i1) => i * i1).Sum();
            }
        ...
        public IEnumerable<string> GetAllPairs(IEnumerable<string> boys, IEnumerable<string> girls) {
               return boys.SelectMany(girl => girls, (boy, girl) => boy + "+" + girl);
            }
